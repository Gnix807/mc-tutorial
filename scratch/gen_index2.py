...
# Read old descriptions
descriptions = {}
with open(index_file, 'r', encoding='utf-8') as f:
    for line in f:
        match = re.search(r'\[.*?\]\(\{\{< relref ".*/(.*?)" >\}\}\)(——.*)', line)
        if match:
            basename = match.group(1).replace('"', '').strip()
            desc = match.group(2).strip()
            descriptions[basename] = desc
...
for ch_dir in ch_dirs:
    ...
    for md_file in md_files:
        ...
        desc = descriptions.get(basename, "——(待补充)")
        
        relref_path = f'/docs/{ch_dir}/{basename}'
        sections.append(f'-   [{file_title}]({{{{< relref "{relref_path}" >}}}}){desc}')
...
print("Restored descriptions and generated new _index.md")
